...
bwa_index = reference_dir+"hg19.fasta"
...
class pipeline():
  ...
  def run_pipeline(self):
    ...
    # Align to reference genome (BWS) and sort (samtools)
    bwa_cmd = [bwa_binary, "mem"]
    bwa_cmd += [bwa_index]
    bwa_cmd += [self.fastq1]
    bwa_cmd += [self.fastq2]
    bwa_cmd += ["-t", self.threads]
    """
    bwa_RG = "@RG\tID:" + self.rg_id  \
        + "\tPL:" + self.rg_pl        \
        + "\tLB:" + self.rg_lb        \
        + '\tSM:' + self.rg_sm
    bwa_cmd += ["-R", "'", bwa_RG, "'"]
    """
    bwa_err = output_folder+self.sample_name+".bwa.err"

    sort_cmd = [samtools_binary, "sort"]
    sort_cmd += ["-@", "8"]
    sorted_bam = output_folder+self.sample_name+".bwa.bam"
    sort_err = output_folder+self.sample_name+".sort.err"

    print sort_cmd
    with open(sorted_bam,"w") as f_sorted_bam:
      with open(sort_err,"w") as f_sort_err:
        with open(bwa_err,"w") as f_bwa_err:
          self.logger.info("Running: "+" ".join(bwa_cmd)+" | "+" ".join(sort_cmd)+" > "+sorted_bam)
          bwa_process = subprocess.Popen(bwa_cmd,
